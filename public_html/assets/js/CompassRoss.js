    ...
    $.fn.extend({
        CompassRose: function(settings){   
            ...
            var sett = $.extend({
                ...
                ,size: 162
                ,arrow: '/img/arrow.png'
                ,compass: '/img/rose.png',
                location: 'locations',
                arrow_id :'arrow',
                compass_class: 'inputCompass',
                image_class: 'imgB1',
                image_classA: 'imgA1',
            }, settings);
            
            var img1 = '<img id="#'+sett.arrow_id+'" class="'+sett.image_class+'" src="'+sett.arrow+'"/>';
            var input = '<input class="'+sett.compass_class+'" type="text" id="'+sett.location+'" name="'+sett.compass_class+'" value="" />';
            var map = '<map name="'+sett.location+'"></map>';
            ...
            img.onload = function() {    
                ...
                $(img).attr('class',sett.image_classA);
                $(img).attr('usemap','#'+ sett.location);
                $(img).attr('width',imageWidth);
                $(img).attr('height',imageHeight);


                $('[name= "'+ sett.location+ '"]').html(
                    '<area shape="rect" coords="'+parseInt(imageWidth*0.41)+','+parseInt(imageHeight*0.15)+','+(parseInt(imageWidth*0.41)+31)+','+(parseInt(imageHeight*0.15)+16)+'" href="" alt="North" id="l1"/>\n\
                ...
                $('[name="'+sett.location+'"]').find('area').each(function(i){
                    $('#'+$(this).attr('id')).bind("click", function(){
                        var pos = - 45*i + 180;
                        direction = $(this).attr('alt');
                      //  $('#orientation-error').data('val',direction)
                        $('#nav-form').next('.invalid-feedback').data('val',direction);
                        $('.'+$(img1).attr('class')).rotate({ angle:0,animateTo:pos,easing: $.easing.easeInOutExpo });                    
                        $('#'+$(input).attr('id')).val( pos );
                    })
                });
                
                $("."+sett.image_class).css('height',parseInt(imageHeight*0.5));
                $("."+sett.image_class).css('top',parseInt(imageHeight*0.26));
                $("."+sett.image_class).css('left',parseInt(imageWidth*0.46));
                //Initial potition
                $("."+sett.image_class).rotate({ angle:0,animateTo:sett.pos,easing: $.easing.easeInOutExpo });
            }
