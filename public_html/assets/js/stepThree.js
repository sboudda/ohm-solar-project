...
$(document).ready(function(){
    ...
    const lng = $('#lng').data("address");
    const lat = $('#lat').data("address");
    var mapOption =  {zoom: 18,
        center: {
            lat: lat,
            lng: lng
        },
    ...
    // set the bounds of the polygon
     const bounds = [
         ...
         { lat: lat-0.0002, lng: lng+ 0.0002 },
            ...
            polygon = new google.maps.Polygon(polygonOptions);
            polygon.setMap(map);
            drawOnMap(map.getCenter(), bounds);
            makerMovable(markers,true);

      // submit handle
      $('#step_three_form_next').click(function(e){
          e.preventDefault();
          $('#step_three_form_area').val(calculateArea());
          //get the borders
          var bounds = polygon.getPath();
          var coord = bounds.getArray();
          var point = [];
          ...
          point.push([coord[2]]);
          point.push([coord[3]]);
          $('#step_three_form_border_a').val(point[0]);
          $('#step_three_form_border_b').val(point[1]);
          $('#step_three_form_border_c').val(point[2]);
          $('#step_three_form_border_d').val(point[3]);

          //submit form
          $('form[name="step_three_form"]').submit();
          return true;
